...
# full stack at Day-1 sizing ($150) — the shakeout before the accelerated 4-day
# eval's clock starts. Bounds all journal queries by the rehearsal epoch (arg
# $1 = epoch ms, else /var/lib/bot/data/eval/rehearsal_epoch's first field, else
# 24 h ago). Prints PASS/FAIL per gate; exits non-zero if any FAIL. The trips/hr
# gate uses the actual elapsed hours, so it adapts to the real rehearsal duration.
...
if [ -z "$T0" ]; then T0=$(( ($(date -u +%s) - 86400) * 1000 )); fi   # 24 h fallback
